...
<div class="row">
    <div class="col-xs-3 col-xs-offset-2">
        ...
        <?=Form::open('admin/photogallery/view', array('enctype' => 'multipart/form-data'))?>
...
<? foreach ($parents_name as $name):?>
        ...
        <?if($item->parent_id == $name->id):?>
            <div class="row">
                ...
                <div class="col-xs-2 text-center">
                    <p>
                        <a class="btn btn-warning btn-sm btn-block" type="button" href="<? echo 'http://'.$_SERVER['SERVER_NAME'].'/admin/photogallery/edit/'.$item->id.'/'.$lang ?>">Редагувати</a>
                    </p>
                    <p>
                        <a class="btn btn-danger btn-sm btn-block" type="button" href="#" onclick="destroy_image('<? echo $item->id?>','<? echo $item->language_id?>')">Видалити</a>
                    </p>
                </div>
            </div>
        <?endif?>
    ...
    <div class="row">
        <div class="col-xs-2 col-xs-offset-5">
            <a class="btn btn-primary btn-sm btn-block" type="button" href="<? echo 'http://'.$_SERVER['SERVER_NAME'].'/admin/photogallery/add/'.$name->id.'/'.$lang ?>">Додати</a>
        </div>
    </div>
