...
class Controller_Backend_Services extends Controller_Backend {
    ...
    public function action_add() {
        ...
        if (isset($_POST['submit'])) {
            ...
            if($post->check()) {
                ...
                if ($_SESSION['counter'] < 3) {
                    $this->redirect('admin/services/add/' . $parent_id . '/' . 'add' . '/' . $subpage_id);
                } else {
                    $_SESSION['counter'] = 0;
                    $this->redirect('admin/services/view/');
                }
            }
        ...
        $content = View::factory('backend/services/v_services_add', array('languages' => $languages,
            'measures' => $measures,
            'parent_id' => $parent_id,
            'item' => $item_add))
                                        ->bind('errors', $errors)
    ...
    public function action_edit()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            if($post->check()) {
                ...
                $this->redirect('admin/services/view/' . $this->lang);
            }
        ...
        $content = View::factory('/backend/services/v_services_edit', array('text' => $subpage->texts->text,
                                                                            'lang' => $this->lang,
    ...
    public function action_view()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            if($post->check()) {
                ...
                ORM::factory('Main')->name_main(4, $this->lang_id, $from_form['text']);
                $this->redirect('admin/services/view/');
            }
        ...
        $content = View::factory('/backend/services/v_services_view', array('parents_name' => $parents_name,
            'items' => $items,
            'lang' => $this->lang,
            'languages' => $languages,
